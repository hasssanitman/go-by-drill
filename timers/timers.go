...
func main() {
	...
	// Give the timer2 enough time to fire, if it ever was going to, to
	// show it is in fact stopped.
	time.Sleep(2 * time.Second)
}

// The first timer will fire ~2s after we start the program, but the
// second should be stopped before it has a chance to fire.
// $ go run timers/timers.go
// timer 1 fired
// timer 2 stopped
