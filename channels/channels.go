...
func main() {
	...
	go func() {
		// Send a value into a channel using the channel <- syntax. Here
		// we send "ping" to the messages channel we made above, from
		// a new goroutine.
		// time.Sleep(3 * time.Second)
		messages <- "ping"
	}()

	// The <-channel syntax receives a value from the channel. Here
	// we’ll receive the "ping" message we sent above and print it
	// out.
	msg := <-messages
	fmt.Println(msg)
...
// receiver are ready. This property allowed us to wait at the end of
// our program for the "ping" message without having to use any
