...
// to stop processing input if it receives a SIGINT. Here’s how to
...
func main() {
	...
	// We could receive from sigs here in the main function, but let’s
	// see how this could also be done in a separate goroutine, to
	// demonstrate a more realistic scenario of graceful shutdown.
	done := make(chan bool, 1)

	// This goroutine executes a blocking receive for signals. When it
	// gets one it’ll print it out and then notify the program that it can
	// finish.
	go func() {
		sig := <-sigs
		fmt.Println()
	...
	// indicated by the goroutine above sending a value on done) and
	// then exit.
...
// When we run this program it will block waiting for a signal. By
// typing ctrl-C (which the terminal shows as ^C) we can send a
// SIGINT signal, causing the program to print interrupt and
// then exit.
// $ go run signals/signals.go
// awaiting signal
// ^C
// interrupt
// exiting
