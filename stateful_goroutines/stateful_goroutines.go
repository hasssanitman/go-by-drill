...
// memory by communicating and having each piece of data
...
// concurrent access. In order to read or write that state, other
// goroutines will send messages to the owning goroutine and
// receive corresponding replies. These readOp and writeOp
...
func main() {
	...
	// channels, responding to requests as they arrive. A response is
	// executed by first performing the requested operation and then
	// sending a value on the response channel resp to indicate
	...
	// constructing a readOp, sending it over the reads channel, and
...
// $ go run stateful_goroutines/stateful_goroutines.go
// readOps:  73974
// writeops:  7451
...
// certain cases though, for example where you have other
// channels involved or when managing multiple such mutexes
// would be error-prone. You should use whichever approach feels
// most natural, especially with respect to understanding the
