...
// and send the corresponding results on results. We’ll sleep a
...
func main() {
	...
	// that the worker goroutines have finished. An alternative way to
	// wait for multiple goroutines is to use a WaitGroup.
	for a := 1; a <= numJobs; a++ {
		<-results
	}
...
// despite doing about 5 seconds of total work because there are 3
// workers operating concurrently.

// $ time go run worker_pools/worker_pools.go
