...
func f1(arg int) (int, error) {
	...
	// A nil value in the error position indicates that there was no
	// error.
	return arg + 3, nil
}

// It’s possible to use custom types as errors by implementing the
// Error() method on them. Here’s a variant on the example
// above that uses a custom type to explicitly represent an
// argument error.
...
func main() {
	...
	// functions. Note that the use of an inline error check on the if
